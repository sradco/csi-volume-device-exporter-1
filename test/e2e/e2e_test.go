...
func TestExporterE2E(t *testing.T) {
	...
	t.Run("metrics endpoint serves prometheus format", func(t *testing.T) {
		...
		if !strings.Contains(content, "csi_volume_device_exporter_discovery_errors_total") {
			t.Error("expected csi_volume_device_exporter_discovery_errors_total in output")
		}
	})
